...
function renderChatMessages(msgs) {
  ...
  list.innerHTML = msgs.map(m => {
    ...
    const time = m.createdAt?.toDate
      ...
      : "";
    // Only the user's own messages can be deleted from this side, and only
    // via press-and-hold (no visible delete button).
    const holdAttrs = (who === "user" && m.id)
      ...
      : "";
    const holdClass = (who === "user" && m.id) ? " chat-bubble-holdable" : "";
