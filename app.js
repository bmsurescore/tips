...
const LANG_STRINGS = {
  sw: {
    ...
    push_status_on: "Arifa za Push: Zimewashwa",
    push_status_off: "Arifa za Push: Zimezimwa",
    push_status_blocked: "Arifa za Push: Zimezuiwa na browser",
    push_status_blocked_hint: "Nenda kwenye mipangilio ya site kwenye browser wako na uruhusu arifa",
    push_status_ios_hint: "Weka app Home Screen kwanza ili upate arifa",
    push_status_enable_btn: "Washa",
    push_status_disable_btn: "Zima",
    ...
    placeholder_name: "mfano: Bmsurescore",
    placeholder_email: "mfano: bmsurescore@gmail.com",
    ...
    blocked_msg: "Akaunti yako imezuiwa kutumia BM SURESCORE.",
    ...
    /* Plain-language "How it works" strip for beginner users (home page) */
    hiw_title: "Jinsi Inavyofanya Kazi",
    hiw_step1_title: "Chagua Free au VIP",
    hiw_step2_title: "Soma tip ya mechi",
    hiw_step3_title: "Angalia Historia",

    /* Pictorial VIP code-unlock guide */
    code_guide_step1: "Pata Msimbo",
    code_guide_step2: "Bandika Msimbo",
    code_guide_step3: "Bonyeza Fungua",
    code_guide_step4: "VIP Inafunguka",
    ...
    tier_single_desc: "Tip moja ya siku, uhakika wa hali ya juu.",
    tier_elite_desc: "Tips kadhaa za siku na uchambuzi wa kina.",
    tier_overunder_desc: "Utabiri wa idadi ya mabao — zaidi au chini ya kiwango fulani.",
    tier_full_desc: "VIP zote tatu kwa bei moja nafuu.",
    ...
    code_in_use: "Msimbo huu tayari unatumika kwenye akaunti nyingine ❌",
    code_accepted: "Msimbo umekubaliwa ✅ Inafungua…",
    code_claimed: "Msimbo huu tayari umeshachukuliwa ❌",
    code_error: "Hitilafu: {msg}",
    code_now_other_device: "Msimbo huu sasa unatumika kwenye akaunti nyingine ❌",
    code_disabled_popup: "Msimbo huu wa VIP umezimwa ❌",
    code_expired_popup: "Msimbo huu wa VIP umeisha muda ⏰",
    code_help_link: "Wasiliana na Admin",

    /* Renewal reminder (shown a few days before a tier's VIP expires) */
    vip_renewal_days: "Inaisha baada ya siku {days}",
    vip_renewal_today: "Inaisha leo",
    btn_renew_now: "Boresha Sasa",
    chat_prefill_renew: "Habari, ningependa kuboresha (renew) {tier} yangu kabla haijaisha.",

    /* VIP history shown in the account panel */
    account_vip_history_title: "Historia ya VIP",
    account_vip_history_empty: "Bado hujawahi kufungua VIP",
  ...
  en: {
    ...
    placeholder_name: "e.g. Bmsurescore",
    placeholder_email: "e.g. bmsurescore@gmail.com",
    ...
    blocked_msg: "Your account has been blocked from accessing BM SURESCORE.",
    ...
    hiw_step1_title: "Pick Free or VIP",
    hiw_step2_title: "Read the match tip",
    hiw_step3_title: "Check History",

    code_guide_step1: "Get Code",
    code_guide_step2: "Paste Code",
    code_guide_step3: "Press Unlock",
    code_guide_step4: "VIP Unlocks",
    ...
    code_now_other_device: "This code is now active on another account ❌",
...
function startBlockedWatcher() {
  ...
  blockedUnsub = onSnapshot(doc(db, "analytics_visitors", myId), snap => {
    ...
    // Admin "Grant VIP" direct-grant pickup — see applyDirectGrants() below.
...
// Push status row inside the bell panel — this is the PERMANENT
// entry point for turning push on, so returning/existing users are
// never limited to the one-shot bottom banner. Works off PushCenter
// (defined in index.html), which wraps the OneSignal SDK.
...
window.showHistoryType = function(type) {
  ...
  // Every time the VIP side is opened, land on a tier's history directly —
  // the tier tab bar is always visible, so switching tiers is one tap.
  if (type === "vip" && !window.__histVipTierChosen) window.showHistVipTier("elite");
};

/* Switch which tier's match history is showing. The tier tab bar (#histVipChoose)
   stays visible the whole time — this only swaps the panel below it, no
   separate "choose" screen and no back-navigation needed. */
...
/* Kept for backward compatibility with any other call sites — resets to
   the default (Elite) tier now that there's no separate picker screen. */
window.backToHistVipCategories = function() {
  window.showHistVipTier("elite");
};
...
/* ══════════════════════════════════════════════════════════
   RENEWAL REMINDER — shown inside the "VIP Active" card once a
   tier is within a few days of expiring, with a one-tap CTA that
   opens chat with a pre-filled renewal message for admin.
   ══════════════════════════════════════════════════════════ */
...
// Inline "Wasiliana na Admin" quick-action shown under an actionable code
// error (invalid/disabled/expired/in-use/claim failure) so the person
// doesn't have to leave the VIP screen to get help.
...
// Records a successful redemption on the user's own analytics_visitors
// doc so it can be shown back to them as "VIP History" in the account
// panel. Best-effort only — never blocks or fails the unlock flow.
  ...
  try {
    ...
    if (!snap.exists()) {
      ...
      if (outcome.status === "disabled") {
        ...
        renderCodeError(msgEl, t('code_claimed'), "elite");
      } else {
...
function renderExtraTierStatus(tier) {
  ...
  if (unlocked) {
    ...
    slot.innerHTML = `
      ...
      ${buildRenewalBanner(expDate, tier)}
      ${banner}`;
...
// ── Admin "Grant VIP" direct-grant pickup ───────────────────────
// The OLD flow still works exactly as before: admin generates a code,
// gives it to the person, they type it into the app themselves
// (unlockVipCode / unlockExtraVipCode above).
//
// This is the NEW flow: from the Users list in Admin.html, an admin can
// grant VIP to a specific user directly. Doing so (a) creates a normal,
// already-redeemed code doc in that tier's codes collection (identical
// shape to a self-redeemed code, so Edit/Release/Pause/Delete all keep
// working on it exactly as they do today), and (b) drops a small pointer
// to that code onto the user's own analytics_visitors/{uid} doc, under
// `directGrants.<tier>`.
//
// This function runs every time that doc's live listener (startBlockedWatcher,
// above) fires. It only needs read access to a document the client is
// already permitted to read in real time — no new Firestore rules and no
// action from the user — so a grant applies within moments of the admin
// clicking "Grant", even if the app is already open.
function applyDirectGrants(directGrants) {
  ...
  Object.keys(directGrants).forEach(tier => {
    ...
    // Each grant is only ever acted on once per code — this stops the
    // listener from re-subscribing on every unrelated snapshot (e.g. a
    // blocked-status change), and stops it from "resurrecting" a code
    // the user (or a later admin action) has since cleared/expired.
...
window.unlockExtraVipCode = async function(tier) {
  ...
  try {
    ...
    else if (outcome.status === "claim_error") { renderCodeError(msgEl, t('code_claimed'), tier); }
    else {
