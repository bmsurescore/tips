...
const LANG_STRINGS = {
  sw: {
    ...
    offline_banner_text: "Hakuna mtandao — unaona taarifa za mwisho ulizohifadhi.",
    offline_banner_back: "✅ Mtandao umerudi",
    offline_overlay_title: "Hakuna Mtandao",
    offline_overlay_text: "Ukiwa mtandaoni, utaona tena utabiri na matokeo ya leo yaliyosasishwa.",
    offline_overlay_waiting: "Inasubiri muunganisho…",
    ...
    chat_send_offline: "Huna mtandao — ujumbe haujatuma. Angalia intaneti/MB zako kisha jaribu tena.",
  ...
  en: {
    ...
    offline_banner_text: "No internet connection — showing your last saved data.",
    offline_banner_back: "✅ Back online",
    offline_overlay_title: "No Internet Connection",
    offline_overlay_text: "Once you're back online, you'll see today's predictions and latest results again.",
    offline_overlay_waiting: "Waiting for connection…",
    ...
    chat_send_offline: "No internet — message wasn't sent. Check your connection/data bundles then try again.",
...
window.sendChatMessage = async function() {
  ...
  // Firestore queues writes locally and resolves addDoc() immediately
  // even while offline (so the message can sync later) — that would
  // make the UI show "sent" when nothing actually reached the admin.
  // Verify a real connection first so we can tell the user honestly.
  const isOnline = await probeRealConnectivity();
  if (!isOnline) {
    chatSending = false;
    sendBtn.disabled = false;
    showToast(t('chat_send_offline'));
    return;
  }
...
// ── Offline handling ────────────────────────────────────────────
// Firestore's onSnapshot listeners serve cached data instantly when
// offline, which would otherwise let a user act on stale odds/scores
// without knowing it. The overlay fully blocks that content while
// offline; the banner is reused only to flash a brief "back online"
// confirmation once the connection returns.
//
// navigator.onLine only reflects whether a network interface is
// switched on — a phone with mobile data turned on but with no
// bundles/airtime still reports onLine === true. We back that up with
// a real network probe. A SINGLE probe target turned out to be
// unreliable on its own: some networks/sandboxed preview tools block
// or don't proxy specific external hosts even though the device has
// perfectly normal internet access, which showed up as a false
// "No Internet Connection" even with real data working. So we race
// several independent probes (two different external hosts + this
// app's own origin) and only call it offline if EVERY one of them
// fails — a false "offline" now requires all three to be unreachable,
// while a false "online" still requires navigator.onLine to agree.
...
let offlineBackTimer = null;
let connectivityCheckInFlight = false;
// Tracks whether we've ever confirmed *real* connectivity since the page
// loaded. Used so the "back online" confirmation banner only flashes after
// a genuine offline→online transition, never on the very first successful
// check right after load.
let hasConfirmedConnectivity = false;

async function probeSingleUrl(url) {
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), CONNECTIVITY_PROBE_TIMEOUT_MS);
...
async function probeRealConnectivity() {
  ...
  if (connectivityCheckInFlight) return null; // a check is already running; caller should skip this tick
  connectivityCheckInFlight = true;
  try {
    await Promise.any(CONNECTIVITY_PROBE_URLS.map(probeSingleUrl));
    return true; // at least one probe got through
  } catch (allFailed) {
    return false; // every probe failed/timed out
  } finally {
...
function applyOnlineUI(isOnline) {
  ...
  clearTimeout(offlineBackTimer);

  if (isOnline) {
    const wasBlocked = !overlay.classList.contains("hidden");
    overlay.classList.add("hidden");
    if (wasBlocked && hasConfirmedConnectivity) {
      // We were genuinely offline before (not just the initial load check)
      // — drop the block, flash the "back online" confirmation.
      banner.classList.remove("hidden");
      banner.classList.add("offline-banner-back");
      banner.querySelector("span[data-i18n]").innerText = t('offline_banner_back');
      offlineBackTimer = setTimeout(() => {
...
async function updateOnlineStatus() {
  const isOnline = await probeRealConnectivity();
  if (isOnline === null) return; // a probe was already in flight, skip this tick
  applyOnlineUI(isOnline);
}

// Block the page IMMEDIATELY and SYNCHRONOUSLY on load, before the async
// connectivity probe (which can take up to CONNECTIVITY_PROBE_TIMEOUT_MS)
// has a chance to run. Without this, a user with no real internet access
// could briefly see/interact with content (or stale cached Firestore data)
// during the window between page load and the probe resolving. The
// overlay only comes down once probeRealConnectivity has actually
// confirmed a working connection — not just navigator.onLine === true.
(function blockUntilConnectivityConfirmed() {
  const overlay = document.getElementById("offlineOverlay");
  const banner  = document.getElementById("offlineBanner");
...
window.addEventListener("offline", () => applyOnlineUI(false));
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "visible") updateOnlineStatus();
});
setInterval(updateOnlineStatus, CONNECTIVITY_POLL_MS);
